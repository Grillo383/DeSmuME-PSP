...
bool DmaController::loadstate(EMUFILE* f)
{
	...
	//HCF Dismount
	read32le((s32*)&dmaCheck,f); 
	read32le((s32*)&running,f); 
	read32le((s32*)&paused,f); 
	read32le((s32*)&triggered,f); 
	...
	if ((adr >> 24) == 4)
	{
		...
		switch(adr)
		{
			...
			case REG_POSTFLG:
				...
				//HCF We don't use external firmware
				/**
