...
{ 
	...
	vecPtr[0] = sceMatrix4Mul(x, y, z, w, matrix);//x * matrix[0] + y * matrix[4] + z * matrix[8] + w * matrix[12];
	vecPtr[1] = sceMatrix4Mul(x, y, z, w, &matrix[1]);//x* matrix[1] + y * matrix[5] + z * matrix[9] + w * matrix[13];
	vecPtr[2] = sceMatrix4Mul(x, y, z, w, &matrix[2]);//x * matrix[2] + y * matrix[6] + z * matrix[10] + w * matrix[14];
	vecPtr[3] = sceMatrix4Mul(x, y, z, w, &matrix[3]);//x * matrix[3] + y * matrix[7] + z * matrix[11] + w * matrix[15];

	//printf("N: %f  O: %f\n", sceMatrix4Mul(x, y, z, w, matrix), vecPtr[0]);
}
...
void MatrixMultVec3x3(const float* matrix, float* vecPtr)
{
	...
	vecPtr[0] = sceMatrix3Mul(x, y, z, matrix);// x * matrix[0] + y * matrix[4] + z * matrix[8];
	vecPtr[1] = sceMatrix3Mul(x, y, z, &matrix[1]);//x * matrix[1] + y * matrix[5] + z * matrix[9];
	vecPtr[2] = sceMatrix3Mul(x, y, z, &matrix[2]);//x * matrix[2] + y * matrix[6] + z * matrix[10];
}
...
void MatrixMultiply(float* matrix, const float* rightMatrix)
{
	...
	fast_memcpy(matrix, tmpMatrix, sizeof(float) * 16);
}
...
inline void vfpuScale(float* matrix, float val) {
	__asm__ volatile (
		"lv.q    C000, %0\n"
		"mtv     %1, S010\n"
...
void MatrixScale(float* matrix, const float* ptr)
{
	...
	/*vfpuScale(&matrix[0], ptr[0]);
	vfpuScale(&matrix[4], ptr[0]);
	vfpuScale(&matrix[8], ptr[0]);*/
...
void MatrixCopy(float* matrixDST, const float* matrixSRC)
{
	...
	fast_memcpy(matrixDST, matrixSRC, sizeof(float) * 16);
}
