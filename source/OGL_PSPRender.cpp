...
static void OGLReset()
{
	...
	memset(_screenColor, 0, sizeof(_screenColor));
}
...
static void GL_ReadFramebuffer()
{
	...
	glReadPixels(0, 0, 256, 192, GL_BGRA_EXT, GL_UNSIGNED_BYTE, _screenColor);
	ENDGL();
...
/*	if (hasShaders)
	{
		...
		u32 rgbToonTable[32];
		for (int i = 0;i < 32;i++) rgbToonTable[i] = RGB15TO32(gfx3d.renderState.u16ToonTable[i], 255);
	...
	//TODO - properly doublebuffer the display lists
	{
		...
		/*	if(lastProjIndex != poly->projIndex) {
			...
			float alpha = poly->getAttributeAlpha() / 31.0f;
			...
			{
				VERT* vert1 = &clippedPoly.clipVerts[j];
				VERT* vert2 = &clippedPoly.clipVerts[j+1];
