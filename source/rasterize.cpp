...
public:
	...
	FORCEINLINE void pixel(int adr,float r, float g, float b, float invu, float invv, float w, float z)
	{
        ...
        //HCF
        shader.materialColor.r = max(0U,min(63U,(unsigned int)u32floor(r)));
		shader.materialColor.g = max(0U,min(63U,(unsigned int)u32floor(g)));
		shader.materialColor.b = max(0U,min(63U,(unsigned int)u32floor(b)));
        /*
	...
	FORCEINLINE void drawscanline(edge_fx_fl *pLeft, edge_fx_fl *pRight) //, bool lineHack)
	{
		...
		*/
		...
		if(x+width > GFX3D_FRAMEBUFFER_WIDTH)
		{
			//HCF No Linehack
			return;
			/*
			...
			*/
			...
			/**
			width = GFX3D_FRAMEBUFFER_WIDTH - x;
	...
	void runscanlines(edge_fx_fl *left, edge_fx_fl *right, bool horizontal)
	{
		...
		//HCF No linehack
		/**
		if ( left->Height == 0 && right->Height == 0 && left->Y<GFX3D_FRAMEBUFFER_HEIGHT && left->Y>=0)
		{
